...
function MainBlogPage({ posts, resetPostForm }) {
  console.log(`${props.posts.posts}`);
  const allPosts = props.posts.posts.map((post) => {
    return (
      <RenderPost
        post={post}
        isLoading={props.posts.isLoading}
        errMess={props.posts.errMess}
      />
    );
  });
  return (
    <React.Fragment>
      ...
      <div className="container-fluid text-center mx-auto">
        <Row>
          <Col xs={12}>
            <AddPostForm resetPostForm={props.resetPostForm} />
          </Col>
        </Row>
      </div>
